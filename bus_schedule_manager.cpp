...
bool BusScheduleManager::CompareSchedules(int index1, int index2) const {
    if (index1 >= 0 && index1 < size_ && index2 >= 0 && index2 < size_) {
        return *schedules_[index1] == *schedules_[index2];
    } else {
        std::cout << "Error: Invalid indices!\n";
        return false;
    }
}

bool BusScheduleManager::IsFirstScheduleEarlier(int index1, int index2) const {
    if (index1 >= 0 && index1 < size_ && index2 >= 0 && index2 < size_) {
        return *schedules_[index1] < *schedules_[index2];
    } else {
        std::cout << "Error: Invalid indices!\n";
