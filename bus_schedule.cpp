...
bool AreSchedulesEqual(const BusSchedule& lhs, const BusSchedule& rhs) {
    return strcmp(lhs.destination_, rhs.destination_) == 0 &&
           strcmp(lhs.bus_number_, rhs.bus_number_) == 0 &&
           strcmp(lhs.departure_time_, rhs.departure_time_) == 0;
}

bool BusSchedule::operator==(const BusSchedule& other) const {
    return strcmp(destination_, other.destination_) == 0 &&
           strcmp(bus_number_, other.bus_number_) == 0 &&
           strcmp(departure_time_, other.departure_time_) == 0;
}
...
bool BusSchedule::operator<(const BusSchedule& other) const {
    return strcmp(departure_time_, other.departure_time_) < 0;
}